    ...
    if (!empty($afm)) {
        ...
        if (check_afm($afm)) {
            ...
            if (file_exists($file)) {
                ...
                if (isset($_POST['amm'])) $given_amm = $_POST['amm'];
                else $given_amm = '';
...
class misthodosia
{
    ...
    private function print_header()
    {
        ...
        if (!empty($rank)) $this->user['ranktxt'] = ' / Βαθμός: ' . $category . ' - ' . $GLOBALS['ranks'][$rank];
        else $this->user['ranktxt'] = '';
...
function print_form($error = 'none')
{
    ...
    echo '
		<form action="' . $_SERVER['PHP_SELF'] . '" method="post">			
            <div class="user_login_container clearfix">
    			<div class="user_form" style="float: left; width: 330px;">
    ...
    echo '<input type="hidden" name="proccess" />
        			<input type="submit" id="ops_submit" value="Συνέχεια" style="padding: 10px 20px; font-size: 16px; font-weight: bold;" />	
                </div>';
    if (!empty($txt['login_below'])) echo '<div style="float: left; width: 400px;">' . $txt['login_below'] . '</div>';
    echo '</div>
		</form>		
	';
}
